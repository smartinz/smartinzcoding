...
namespace Rhino.Security.Controllers
{
	...
	{
		private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(Rhino.Security.Controllers.OperationController));
		private readonly Rhino.Security.Data.OperationRepository _repository;
		...
		public OperationController(Conversation.IConversation conversation, AutoMapper.IMappingEngine mapper, Rhino.Security.Data.OperationRepository repository, Nexida.Infrastructure.IValidator validator, Nexida.Infrastructure.IStringConverter<Rhino.Security.Model.Operation> stringConverter)
		{
		...
		public ActionResult Save(Rhino.Security.Dtos.OperationDto item)
		{
			using(_conversation.SetAsCurrent())
			{
				var itemMapped = _mapper.Map<Rhino.Security.Dtos.OperationDto, Rhino.Security.Model.Operation>(item);
		...
		public ActionResult Load(string stringId)
		{
			using(_conversation.SetAsCurrent())
			{
				...
				var itemDto = _mapper.Map<Rhino.Security.Model.Operation, Rhino.Security.Dtos.OperationDto>(item);
				return Json(itemDto);
			}
		}
			...
			using(_conversation.SetAsCurrent())
			{
				...
				public ActionResult Search(System.Guid? id, string name, string comment, int start, int limit, string sort, string dir)
				{
					...
					using(_conversation.SetAsCurrent())
					{
						...
						var dtos = _mapper.Map<IEnumerable<Rhino.Security.Model.Operation>, Rhino.Security.Dtos.OperationDto[]>(items);
