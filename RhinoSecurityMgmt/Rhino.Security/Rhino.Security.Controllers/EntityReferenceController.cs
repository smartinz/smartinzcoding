...
namespace Rhino.Security.Controllers
{
	...
	{
		private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(Rhino.Security.Controllers.EntityReferenceController));
		private readonly Rhino.Security.Data.EntityReferenceRepository _repository;
		...
		public EntityReferenceController(Conversation.IConversation conversation, AutoMapper.IMappingEngine mapper, Rhino.Security.Data.EntityReferenceRepository repository, Nexida.Infrastructure.IValidator validator, Nexida.Infrastructure.IStringConverter<Rhino.Security.Model.EntityReference> stringConverter)
		{
		...
		public ActionResult Save(Rhino.Security.Dtos.EntityReferenceDto item)
		{
			using(_conversation.SetAsCurrent())
			{
				var itemMapped = _mapper.Map<Rhino.Security.Dtos.EntityReferenceDto, Rhino.Security.Model.EntityReference>(item);
		...
		public ActionResult Load(string stringId)
		{
			using(_conversation.SetAsCurrent())
			{
				...
				var itemDto = _mapper.Map<Rhino.Security.Model.EntityReference, Rhino.Security.Dtos.EntityReferenceDto>(item);
				return Json(itemDto);
			}
		}
			...
			using(_conversation.SetAsCurrent())
			{
				...
				public ActionResult Search(System.Guid? id, System.Guid? entitySecurityKey, int start, int limit, string sort, string dir)
				{
					...
					using(_conversation.SetAsCurrent())
					{
						...
						var dtos = _mapper.Map<IEnumerable<Rhino.Security.Model.EntityReference>, Rhino.Security.Dtos.EntityReferenceDto[]>(items);
