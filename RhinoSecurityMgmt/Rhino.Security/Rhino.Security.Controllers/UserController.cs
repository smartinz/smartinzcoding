...
namespace Rhino.Security.Controllers
{
	...
	{
		private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(Rhino.Security.Controllers.UserController));
		...
		public UserController(Conversation.IConversation conversation, AutoMapper.IMappingEngine mapper, Rhino.Security.Data.UserRepository repository, Nexida.Infrastructure.IValidator validator, Nexida.Infrastructure.IStringConverter<Rhino.Security.Model.User> stringConverter)
		{
		...
		public ActionResult Save(Rhino.Security.Dtos.UserDto item)
		{
			using(_conversation.SetAsCurrent())
			{
				var itemMapped = _mapper.Map<Rhino.Security.Dtos.UserDto, Rhino.Security.Model.User>(item);
		...
		public ActionResult Load(string stringId)
		{
			using(_conversation.SetAsCurrent())
			{
				...
				var itemDto = _mapper.Map<Rhino.Security.Model.User, Rhino.Security.Dtos.UserDto>(item);
				return Json(itemDto);
			}
		}
			...
			using(_conversation.SetAsCurrent())
			{
				...
				public ActionResult Search(System.Int64? id, string name, int start, int limit, string sort, string dir)
				{
					...
					using(_conversation.SetAsCurrent())
					{
						...
						var dtos = _mapper.Map<IEnumerable<Rhino.Security.Model.User>, Rhino.Security.Dtos.UserDto[]>(items);
