...
namespace Rhino.Security.Controllers
{
	...
	{
		private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(Rhino.Security.Controllers.EntityTypeController));
		private readonly Rhino.Security.Data.EntityTypeRepository _repository;
		...
		public EntityTypeController(Conversation.IConversation conversation, AutoMapper.IMappingEngine mapper, Rhino.Security.Data.EntityTypeRepository repository, Nexida.Infrastructure.IValidator validator, Nexida.Infrastructure.IStringConverter<Rhino.Security.Model.EntityType> stringConverter)
		{
		...
		public ActionResult Save(Rhino.Security.Dtos.EntityTypeDto item)
		{
			using(_conversation.SetAsCurrent())
			{
				var itemMapped = _mapper.Map<Rhino.Security.Dtos.EntityTypeDto, Rhino.Security.Model.EntityType>(item);
		...
		public ActionResult Load(string stringId)
		{
			using(_conversation.SetAsCurrent())
			{
				...
				var itemDto = _mapper.Map<Rhino.Security.Model.EntityType, Rhino.Security.Dtos.EntityTypeDto>(item);
				return Json(itemDto);
			}
		}
			...
			using(_conversation.SetAsCurrent())
			{
				...
				public ActionResult Search(System.Guid? id, string name, int start, int limit, string sort, string dir)
				{
					...
					using(_conversation.SetAsCurrent())
					{
						...
						var dtos = _mapper.Map<IEnumerable<Rhino.Security.Model.EntityType>, Rhino.Security.Dtos.EntityTypeDto[]>(items);
