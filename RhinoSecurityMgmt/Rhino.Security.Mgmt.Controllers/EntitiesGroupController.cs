...
namespace Rhino.Security.Mgmt.Controllers
{
	...
	{
		private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(typeof(Rhino.Security.Mgmt.Controllers.EntitiesGroupController));
		private readonly Rhino.Security.Mgmt.Data.EntitiesGroupRepository _repository;
		...
		public EntitiesGroupController(Conversation.IConversation conversation, AutoMapper.IMappingEngine mapper, Rhino.Security.Mgmt.Data.EntitiesGroupRepository repository, Nexida.Infrastructure.IValidator validator, Nexida.Infrastructure.IStringConverter<Rhino.Security.Model.EntitiesGroup> stringConverter)
		{
		...
		public ActionResult Save(Rhino.Security.Mgmt.Dtos.EntitiesGroupDto item)
		{
			using(_conversation.SetAsCurrent())
			{
				var itemMapped = _mapper.Map<Rhino.Security.Mgmt.Dtos.EntitiesGroupDto, Rhino.Security.Model.EntitiesGroup>(item);
		...
		public ActionResult Load(string stringId)
		{
			using(_conversation.SetAsCurrent())
			{
				...
				var itemDto = _mapper.Map<Rhino.Security.Model.EntitiesGroup, Rhino.Security.Mgmt.Dtos.EntitiesGroupDto>(item);
				return Json(itemDto);
			}
		}
			...
			using(_conversation.SetAsCurrent())
			{
				...
				public ActionResult Search(System.Guid? id, string name, int start, int limit, string sort, string dir)
				{
					...
					using(_conversation.SetAsCurrent())
					{
						...
						var dtos = _mapper.Map<IEnumerable<Rhino.Security.Model.EntitiesGroup>, Rhino.Security.Mgmt.Dtos.EntitiesGroupDto[]>(items);
